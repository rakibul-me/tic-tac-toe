  ...
  if (withComputer && !com) {
    ...
    if (level == 1) {
      ...
      if (rndInt(1, 3) == 2) {
        for (i = 0; i < length; i++) {
          if (
            getelem(wins[i][0]).innerText == "O" &&
            getelem(wins[i][1]).innerText == "O" &&
            getelem(wins[i][2]).innerText == ""
          ) {
            setTimeout(() => {
            ...
            getelem(wins[i][0]).innerText == "O" &&
            getelem(wins[i][2]).innerText == "O" &&
            getelem(wins[i][1]).innerText == ""
          ) {
            setTimeout(() => {
      ...
      if (rndInt(1, 4) == 2) {
        for (i = 0; i < length; i++) {
          if (
            getelem(wins[i][0]).innerText == "X" &&
            getelem(wins[i][1]).innerText == "X" &&
            getelem(wins[i][2]).innerText == ""
          ) {
            setTimeout(() => {
              handleClick(wins[i][2], true);
      ...
      setTimeout(() => {
        handleClick(
          Number(
            empty[rndInt(0, empty.length - 1)].getAttribute("id").substring(1)
          ),
          true
        );
      }, 700);
...
function checkWinner(turn) {
  ...
  for (i = 0; i < length; i++) {
    if (
      getelem(wins[i][0]).innerText == turn &&
      getelem(wins[i][1]).innerText == turn &&
      getelem(wins[i][2]).innerText == turn
    ) {
      winnerCheckedTimes++;
      winningLine = i;
      return turn;
...
function drawWinningLine(line) {
  let w = document.getElementsByClassName("wrapper")[0].clientWidth;
  let elem = document.getElementById("winningLine");
  elem.style.display = "inline-block";
  switch (line) {
    case 0: {
      elem.style.left = 0;
      elem.style.top = w / 4 - 2.5 - 20 + "px";
      elem.style.transform = "rotateZ(0deg)";
      break;
    }
    case 1: {
      elem.style.left = 0;
      elem.style.top = (2 * w) / 4 - 2.5 + "px";
      elem.style.transform = "rotateZ(0deg)";
      break;
    }
    case 2: {
      elem.style.left = 0;
      elem.style.top = (3 * w) / 4 + 20 - 2.5 + "px";
      elem.style.transform = "rotateZ(0deg)";
      break;
    }
    case 3: {
      elem.style.top = 0;
      elem.style.left = w / 4 - 20 + "px";
      elem.style.transform = "rotateZ(90deg)";
      break;
    }
    case 4: {
      elem.style.top = 0;
      elem.style.left = (2 * w) / 4 + "px";
      elem.style.transform = "rotateZ(90deg)";
      break;
    }
    case 5: {
      elem.style.top = 0;
      elem.style.left = (3 * w) / 4 + 20 + "px";
      elem.style.transform = "rotateZ(90deg)";
      break;
    }
    case 6: {
    ...
    case 7: {
      elem.style.top = 0;
      elem.style.left = "100%";
      elem.style.transform = "rotateZ(135deg)";
      break;
    }
  }
...
function restart() {
  ...
  document.getElementById('prevent').style.display = 'none'; document.getElementById("winningLine").style.display = "none";
...
function createFriendGame() {
  document.getElementById("pX").innerText =
